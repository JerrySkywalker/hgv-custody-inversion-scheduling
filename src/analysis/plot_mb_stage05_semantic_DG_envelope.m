...
fig = create_managed_figure(local_getfield_or(options, 'cfg', []), ...
    'Color', 'w', 'Name', 'MB Stage05 Semantic DG', 'Position', [140 140 1100 700]);
ax = axes(fig);
hold(ax, 'on');
...
for idx = 1:numel(i_list)
    ...
    Ti = sortrows(Ti, 'Ns');
    valid_mask = isfinite(Ti.Ns) & isfinite(Ti.max_D_G_min);
    Ti = Ti(valid_mask, :);
    if isempty(Ti)
        continue;
    elseif numel(unique(Ti.Ns)) < 2
        plot(ax, Ti.Ns, Ti.max_D_G_min, 's', ...
...
ylabel(ax, 'max raw D_G^{min} under fixed i');
title(ax, sprintf('MB Control (Stage05 Semantics): raw D_G^{min} envelope versus N_s at h = %.0f km', h_km));
legend(ax, 'Location', 'eastoutside');
grid(ax, 'on');
box(ax, 'on');
set(ax, 'FontSize', 13);
apply_mb_plot_domain_guardrail(ax, envelope_table.Ns, envelope_table.max_D_G_min, struct( ...
    'auto_ylim', true, ...
    'empty_message', 'No feasible point found within current search domain', ...
