...
for idx = 1:numel(run_output.runs)
    ...
    history_view_spec = struct( ...
        ...
        'figure_name', sprintf('MB_closedD_passratio_historyFull_%s_%s.png', h_label, sensor_group));
    ...
    write_mb_plot_domain_sidecar(pass_alias_png, "effective_full_range", "effective_search_domain", local_capture_axis_xlim(fig_pass_effective), ...
        build_mb_passratio_view_sidecar_fields(fig_pass_effective, run.aggregate.passratio_phasecurve, pass_effective_csv, "effective_full_range", pass_windows.effective_full_range, struct(), struct('figure_family', "closedD_passratio")));
    ...
    local_surface = run.aggregate.requirement_surface_iP;
    global_surface = build_mb_global_skeleton_heatmap_surface(local_surface, search_domain, struct('h_km', run.h_km, 'family_name', string(run.family_name)));
...
search_domain = struct( ...
    ...
    'inclination_grid_deg', reshape(local_getfield_or(history_domain, 'inclination_grid_deg', local_getfield_or(run_output.options, 'i_grid_deg', unique(run.design_table.i_deg, 'sorted'))), 1, []), ...
