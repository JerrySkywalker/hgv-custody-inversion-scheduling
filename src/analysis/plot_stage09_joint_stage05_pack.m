...
function [metric_view, metric_frontiers, cfg] = local_unpack_inputs(in)
    ...
    if ~isstruct(in)
        error('plot_stage09_joint_stage05_pack:InvalidInput', 'Input must be a struct.');
    end
    ...
    if ~isfield(metric_view, 'joint') || ~isstruct(metric_view.joint) || ~isfield(metric_view.joint, 'table') || ~istable(metric_view.joint.table)
        error('plot_stage09_joint_stage05_pack:InvalidMetricView', ...
            'Expected metric_view.joint.table to be a table.');
    end
...
function cfg = local_pick_cfg(in)
    ...
    error('plot_stage09_joint_stage05_pack:MissingCfg', ...
