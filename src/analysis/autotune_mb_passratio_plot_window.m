...
manifest = build_mb_cache_manifest("autotune_probe", mfilename, struct( ...
    'semantic_mode', semantic_mode, ...
    'sensor_group', sensor_group, ...
    'height_km', height_km, ...
    'P_grid', reshape(local_getfield_or(profile, 'P_values', local_getfield_or(profile, 'P_grid', [])), 1, []), ...
    'T_grid', reshape(local_getfield_or(profile, 'T_values', local_getfield_or(profile, 'T_grid', [])), 1, []), ...
    'plot_xlim_ns', reshape(local_getfield_or(profile, 'Ns_xlim_plot', local_getfield_or(profile, 'plot_xlim_ns', [])), 1, []), ...
    'cache_tag', string(local_getfield_or(cache_cfg, 'tag', "mb_auto_plot_tune"))), struct( ...
    'cache_namespace', "mb_autotune_probe", ...
    'semantic_mode', semantic_mode, ...
    ...
    'plot_domain', struct( ...
        'plot_xlim_ns', reshape(local_getfield_or(profile, 'Ns_xlim_plot', local_getfield_or(profile, 'plot_xlim_ns', [])), 1, [])), ...
    'search_profile_name', string(local_getfield_or(profile, 'name', "")), ...
    'profile_mode', string(local_getfield_or(profile, 'profile_mode', "")), ...
    'Ns_grid', [local_getfield_or(profile, 'ns_search_min', NaN), local_getfield_or(profile, 'ns_search_max', NaN), local_getfield_or(profile, 'ns_search_step', NaN)], ...
    'P_grid', reshape(local_getfield_or(profile, 'P_values', local_getfield_or(profile, 'P_grid', [])), 1, []), ...
    'T_grid', reshape(local_getfield_or(profile, 'T_values', local_getfield_or(profile, 'T_grid', [])), 1, []), ...
    'expand_blocks', local_getfield_or(profile, 'Ns_expand_blocks', []), ...
    'Ns_hard_max', local_getfield_or(profile, 'Ns_hard_max', NaN), ...
    'height_km', height_km, ...
    'cache_tag', string(local_getfield_or(cache_cfg, 'tag', "mb_auto_plot_tune")), ...
    'compatible_with_plot_only_changes', false, ...
...
if reuse_tune_cache
    ...
    if loaded.hit
        ...
        if local_score_result_needs_refresh(score_result)
            ...
            save_mb_cache_with_manifest(cache_file, struct( ...
                'profile', profile, ...
                'phasecurve_table', candidate_phasecurve, ...
                'score_result', score_result), manifest);
        end
        cache_hit = true;
        return;
    end
