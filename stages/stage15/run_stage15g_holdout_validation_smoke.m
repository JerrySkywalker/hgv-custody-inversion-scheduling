function out = run_stage15g_holdout_validation_smoke()
%RUN_STAGE15G_HOLDOUT_VALIDATION_SMOKE
% Hold-out validation for staticworld atlas:
% train on early target_ids, test on held-out target_ids.
...
split = stage15_split_staticworld_dataset_holdout_by_target(dataset);
...
out_root = fullfile(pwd, 'outputs', 'stage', 'stage15', 'staticworld_templates3d_holdout');
if ~exist(out_root, 'dir'); mkdir(out_root); end

txt_tpl = fullfile(out_root, 'stage15g_holdout_template_family_summary.txt');
txt_val = fullfile(out_root, 'stage15g_holdout_validation_summary.txt');
mat_tpl = fullfile(out_root, 'stage15g_holdout_template_family.mat');
mat_val = fullfile(out_root, 'stage15g_holdout_validation_result.mat');
...
disp('=== Stage15-G Holdout Validation Smoke ===')
...
disp(['[stage15g-holdout] tpl text : ', txt_tpl]);
disp(['[stage15g-holdout] val text : ', txt_val]);
disp(['[stage15g-holdout] tpl mat  : ', mat_tpl]);
disp(['[stage15g-holdout] val mat  : ', mat_val]);
