...
%RUN_CH5_PHASE2_INNERLOOP  Phase 2 runner for inner-loop and NIS shell.
...
cfg.phase_name = 'phase2';

out_root = fullfile(pwd, 'outputs', 'cpt5', 'phase2');
...
fig_path = fullfile(fig_dir, 'phase2_inner_nis_timeline.png');
fig = plot_inner_nis_timeline(inner, fig_path); %#ok<NASGU>
close all

log_path = fullfile(log_dir, 'phase2_innerloop_log.txt');
log_lines = {
...
mat_path = fullfile(mat_dir, 'phase2_innerloop.mat');
save(mat_path, 'cfg', 'caseData', 'inner');

if verbose
    disp('=== Chapter 5 Phase 2 Inner Loop Summary ===')
    ...
    disp(['[phase2] fig  : ', fig_path]);
    disp(['[phase2] log  : ', log_path]);
    disp(['[phase2] mat  : ', mat_path]);
end
