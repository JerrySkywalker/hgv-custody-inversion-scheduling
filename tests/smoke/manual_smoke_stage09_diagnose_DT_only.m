function out = manual_smoke_stage09_diagnose_DT_only(cfg)
%MANUAL_SMOKE_STAGE09_DIAGNOSE_DT_ONLY
% Stage09 diagnosis smoke:
%   DG + DT + pass_ratio active
%   DA relaxed to 0
% Purpose:
%   identify which Stage05-compatible points are killed by D_T.
    ...
    cfg.stage09.run_tag = 'inverse_stage09_diagnose_DT_only';
    ...
    fprintf('\n[DT_ONLY] Running Stage09 diagnose smoke...\n');
    out = manual_smoke_stage09_stage05_aligned_fullscan(cfg);
    out.diag = stage09_dump_metric_diagnosis(out, 'DT_only', 15);

    fprintf('\n[DT_ONLY] Recommended CSVs:\n');
    fprintf('diag table   : %s\n', string(out.diag.files.diag_csv));
    fprintf('killed by DT : %s\n', string(out.diag.files.killed_dt_csv));
