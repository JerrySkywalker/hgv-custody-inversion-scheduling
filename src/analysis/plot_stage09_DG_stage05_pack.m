...
% Thin wrapper for DG Stage05-style ninepack plotting.
    ...
    pack = plot_stage09_metric_stage05_ninepack(metric_view.DG.table, metric_frontiers.DG, 'DG', cfg, mode_tag);
end
...
function [metric_view, metric_frontiers, cfg] = local_unpack_inputs(in)
    ...
    if ~isstruct(in)
        error('plot_stage09_DG_stage05_pack:InvalidInput', 'Input must be a struct.');
    end
    ...
    if ~isfield(metric_view, 'DG') || ~isstruct(metric_view.DG) || ~isfield(metric_view.DG, 'table') || ~istable(metric_view.DG.table)
        error('plot_stage09_DG_stage05_pack:InvalidMetricView', ...
            'Expected metric_view.DG.table to be a table.');
    end
...
function cfg = local_pick_cfg(in)
    ...
    error('plot_stage09_DG_stage05_pack:MissingCfg', ...
