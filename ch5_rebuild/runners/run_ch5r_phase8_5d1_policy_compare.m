function out = run_ch5r_phase8_5d1_policy_compare()
%RUN_CH5R_PHASE8_5D1_POLICY_COMPARE
% R8.5d.1a:
%   Compare three policy traces on aligned lightweight bubble-effect chain:
%     1) PTA
%     2) observability_family (CN proxy)
%     3) current_method (R5-real normalized selection_trace)
...
rep_pta = compute_bubble_metrics_from_selection_trace_simple(ch5case, trace_pta, 'PTA');
rep_obs = compute_bubble_metrics_from_selection_trace_simple(ch5case, trace_obs, 'observability_family');
rep_cur = compute_bubble_metrics_from_selection_trace_simple(ch5case, trace_cur, 'current_method');
...
compare_table = table( ...
    ...
    [rep_pta.summary.mean_MG_proxy; rep_obs.summary.mean_MG_proxy; rep_cur.summary.mean_MG_proxy], ...
    [rep_pta.summary.min_MG_proxy; rep_obs.summary.min_MG_proxy; rep_cur.summary.min_MG_proxy], ...
    [rep_pta.summary.max_MG_proxy; rep_obs.summary.max_MG_proxy; rep_cur.summary.max_MG_proxy], ...
    [rep_pta.summary.mean_MG_norm; rep_obs.summary.mean_MG_norm; rep_cur.summary.mean_MG_norm], ...
...
out_dir = fullfile(base_out, 'phaseR8_5d1_policy_compare');
if ~exist(out_dir, 'dir')
    mkdir(out_dir);
end

stamp = char(datetime('now', 'Format', 'yyyyMMdd_HHmmss'));
csv_file = fullfile(out_dir, ['phaseR8_5d1_policy_compare_' stamp '.csv']);
mat_file = fullfile(out_dir, ['phaseR8_5d1_policy_compare_' stamp '.mat']);
md_file  = fullfile(out_dir, ['phaseR8_5d1_policy_compare_' stamp '.md']);
...
disp(' ')
disp('=== [ch5r:R8.5d.1a] aligned policy compare summary ===')
disp(compare_table)
...
lines = {};
lines{end+1} = '# Phase R8.5d.1a aligned policy compare';
lines{end+1} = '';
...
for i = 1:height(compare_table)
    lines{end+1} = sprintf('- %s: bubble_steps=%g, bubble_time_s=%g, switch_count=%g, resource_score=%g, mean_MG_proxy=%g, min_MG_proxy=%g, max_MG_proxy=%g, mean_MG_norm=%g, n_missing_pair_steps=%g', ...
        ...
        compare_table.mean_MG_proxy(i), ...
        compare_table.min_MG_proxy(i), ...
        compare_table.max_MG_proxy(i), ...
        compare_table.mean_MG_norm(i), ...
