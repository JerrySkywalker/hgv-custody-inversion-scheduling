...
for idx = 1:numel(run_output.runs)
    ...
    heat_global_csv = fullfile(paths.tables, sprintf('MB_closedD_minimumNs_heatmap_globalReplay_%s_%s.csv', h_label, sensor_group));
    ...
    heat_state_global_csv = fullfile(paths.tables, sprintf('MB_closedD_heatmap_stateMap_globalReplay_%s_%s.csv', h_label, sensor_group));
    ...
    fig_heat_global = plot_mb_fixed_h_requirement_heatmap_iP(global_surface, style, struct( ...
        ...
        'figure_style', local_getfield_or(plot_options, 'figure_style', struct())));
    local_retitle(fig_heat_global, sprintf('closedD Global Replay Minimum Feasible Constellation Heatmap at h = %.0f km [%s]', run.h_km, sensor_label));
    heat_global_png = fullfile(paths.figures, sprintf('MB_closedD_minimumNs_heatmap_globalReplay_%s_%s.png', h_label, sensor_group));
    ...
    fig_heat_state_global = plot_mb_fixed_h_requirement_heatmap_iP(global_surface, style, struct( ...
        ...
        'figure_style', local_getfield_or(plot_options, 'figure_style', struct())));
    local_retitle(fig_heat_state_global, sprintf('closedD Global Replay Heatmap State Map at h = %.0f km [%s]', run.h_km, sensor_label));
    heat_state_global_png = fullfile(paths.figures, sprintf('MB_closedD_heatmap_stateMap_globalReplay_%s_%s.png', h_label, sensor_group));
