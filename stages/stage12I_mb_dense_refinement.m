...
%STAGE12I_MB_DENSE_REFINEMENT Run local high-resolution Stage09 scans around key MB parameter planes.
...
if ~isfield(meta, 'dense_refinement_enabled') || ~logical(meta.dense_refinement_enabled)
    return;
end

dense_cfg = local_dense_cfg(meta, pool, minimum_design_table);
fprintf('[MB][dense] requirement grid: %d h x %d i x %d P x %d T.\n', ...
...
fprintf('[MB][dense] requirement/gap local truth finished: %d designs in %.2fs.\n', ...
...
fprintf('[MB][dense] phasecurve local truth finished: %d designs in %.2fs.\n', ...
...
out.summary = struct( ...
    ...
    'minimum_shell_Ns', local_first_value(minimum_design_table, 'Ns'), ...
    'requirement_design_count', height(requirement_design_table), ...
    'phasecurve_design_count', height(phasecurve_design_table), ...
    'dense_requirement_i_deg', dense_cfg.requirement_i_deg, ...
    'dense_requirement_P', dense_cfg.requirement_P, ...
    'dense_requirement_h_km', dense_cfg.requirement_h_km, ...
    'dense_requirement_T', dense_cfg.requirement_T, ...
    'dense_phasecurve_i_deg', dense_cfg.phasecurve_i_deg, ...
    'dense_phasecurve_P', dense_cfg.phasecurve_P, ...
    'dense_phasecurve_T', dense_cfg.phasecurve_T, ...
    'dense_include_hi', dense_cfg.include_hi, ...
    ...
    'note', "Dense refinement reuses the current Stage09 truth kernel on local explicit design tables and does not trigger a new full MB global scan.");
end
