function out = run_ch5r_phase10_li_backend_closedloop(overrides)
%RUN_CH5R_PHASE10_LI_BACKEND_CLOSEDLOOP
% R10 Phase A:
% - reuse R9 inner loop shell
% - replace rolling scheduler with Li-style fixed-interval relay scheduler

if nargin < 1 || isempty(overrides)
    overrides = struct();
end

cfg = default_ch5r_r10_params();
cfg = local_apply_overrides(cfg, overrides);

addpath(fullfile(pwd, 'ch5_rebuild', 'r9_inner'));
addpath(fullfile(pwd, 'ch5_rebuild', 'r10_li'));
...
for k = 1:Nt
    ...
    elseif k > last_valid_center
        ...
        if isempty(current_pair) || (k > current_interval_end)
            ...
            current_interval_end = interval_sel.k1;
        ...
        sel.k = k;
        sel.time_s = ch5case.t_s(k);
        sel.pair = current_pair;
        sel.score = interval_sel.score;
        sel.name = 'r10_li_interval_hold';
        sel.eval = interval_sel;
        sel.n_pairs = size(pair_list,1);
    ...
    if cfg.ch5r.r10.log.enable
        do_log = (k == 1) || (k == Nt) || ((k <= last_valid_center) && mod(k-1, cfg.ch5r.r10.interval_steps) == 0);
        if do_log
            msg = sprintf('[R10][k=%d/%d] rmsePos=%.6g', k, Nt, rmse_pos_km(k));
            if ~isempty(sel.pair)
                if isnan(sel.score)
...
result.r10_tracking = struct();
result.r10_tracking.rmse_pos_km_series = rmse_pos_km;
result.r10_tracking.mean_rmse_pos_km = sqrt(mean(rmse_pos_km.^2, 'omitnan'));
result.r10_tracking.final_rmse_pos_km = rmse_pos_km(end);
result.r10_tracking.interval_count = interval_count;
result.r10_tracking.xhat_hist = xhat_hist;
...
disp(' ')
disp('=== [ch5r:R10-real] Li-style interval backend summary ===')
...
disp(['mean RMSE pos (km)   : ' num2str(result.r10_tracking.mean_rmse_pos_km, '%.12g')])
disp(['final RMSE pos (km)  : ' num2str(result.r10_tracking.final_rmse_pos_km, '%.12g')])
disp(['interval count       : ' num2str(result.r10_tracking.interval_count)])
...
if isfield(overrides, 'save_outputs')
    cfg.ch5r.r10.save_outputs = logical(overrides.save_outputs);
end
