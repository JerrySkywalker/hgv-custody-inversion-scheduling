...
if isfield(cache_summary_artifacts, 'fresh_manifest_csv') && strlength(cache_summary_artifacts.fresh_manifest_csv) > 0
    result.tables.fresh_recompute_manifest = cache_summary_artifacts.fresh_manifest_csv;
end
...
artifacts = struct('reuse_csv', "", 'signature_csv', "", 'fresh_manifest_csv', "");
reuse_rows = cell(0, 1);
signature_rows = cell(0, 1);
fresh_rows = cell(0, 1);
force_fresh_enabled = logical(local_getfield_or(meta, 'force_fresh', local_getfield_or(local_getfield_or(meta, 'cache_profile', struct()), 'force_fresh', false)));
plot_recomputed = logical(local_getfield_or(meta, 'regenerate_all_export', false) || ~logical(local_getfield_or(local_getfield_or(meta, 'cache_profile', struct()), 'reuse_plotting', true)));
...
for idx = 1:numel(run_outputs)
    ...
    for idx_record = 1:numel(cache_records)
        ...
        fresh_rows{end + 1, 1} = { ... %#ok<AGROW>
            string(sprintf('%s/%s/%s/h%d', char(semantic_mode), char(sensor_group), char(string(local_getfield_or(record, 'family_name', ""))), round(local_getfield_or(record, 'h_km', NaN)))), ...
            logical(local_getfield_or(record, 'cache_hit', false)), ...
            string(local_resolve_fresh_recompute_reason(record, force_fresh_enabled)), ...
            logical(force_fresh_enabled), ...
            logical(force_fresh_enabled || ~logical(local_getfield_or(record, 'cache_hit', false))), ...
            logical(plot_recomputed), ...
            double(local_getfield_or(record, 'h_km', NaN))};
    end

    if string(local_getfield_or(run_outputs(idx), 'mode', "")) == "legacyDG"
        stage_inputs_rebuilt = logical(local_getfield_or(local_getfield_or(run_output, 'options', struct()), 'force_rebuild_inputs', false));
        fresh_rows{end + 1, 1} = { ... %#ok<AGROW>
            "legacyDG/static_stage_input_chain/" + string(local_getfield_or(run_output.sensor_group, 'name', "")), ...
            ~stage_inputs_rebuilt, ...
            string(local_static_input_reason(stage_inputs_rebuilt)), ...
            logical(force_fresh_enabled), ...
            logical(stage_inputs_rebuilt), ...
            false, ...
            NaN};
        fresh_rows{end + 1, 1} = { ... %#ok<AGROW>
            "legacyDG/static_stage01_seed/" + string(local_getfield_or(run_output.sensor_group, 'name', "")), ...
            true, ...
            "static_stage01_seed_reused_only", ...
            logical(force_fresh_enabled), ...
            false, ...
            false, ...
            NaN};
...
if ~isempty(fresh_rows)
    fresh_table = cell2table(vertcat(fresh_rows{:}), 'VariableNames', { ...
        'artifact_name', 'cache_reused', 'reason', 'force_fresh_enabled', ...
        'semantic_recomputed', 'plot_recomputed', 'height_km'});
    artifacts.fresh_manifest_csv = string(fullfile(paths.tables, 'fresh_recompute_manifest.csv'));
    milestone_common_save_table(fresh_table, artifacts.fresh_manifest_csv);
end
end

function reason = local_resolve_fresh_recompute_reason(record, force_fresh_enabled)
reason = string(local_getfield_or(record, 'reason', ""));
if force_fresh_enabled
...
function reason = local_static_input_reason(stage_inputs_rebuilt)
if stage_inputs_rebuilt
    reason = "force_fresh_stage05_input_chain_rebuilt";
else
    reason = "static_stage_input_chain_reused";
end
end
