function out = manual_compare_stage09_closedd_smallset()
startup;
...
% Engine manual-regression cfg: full baseline + profile overlay
...
for k = 1:n
    ...
    % Engine ClosedD path
    % ---------------------------
    engine_eval = evaluate_design_point_closedd( ...
    ...
    legacy_rows(k).DA_rob = legacy_eval.DA_rob;
    legacy_rows(k).DT_rob = legacy_eval.DT_rob;
    ...
    engine_rows(k).DA_rob = engine_eval.DA_rob;
    engine_rows(k).DT_rob = engine_eval.DT_rob;
...
legacy_tbl = renamevars(legacy_tbl, ...
    {'DG_rob','DA_rob','DT_rob','joint_margin','pass_ratio','feasible_flag'}, ...
    {'legacy_DG_rob','legacy_DA_rob','legacy_DT_rob','legacy_joint_margin','legacy_pass_ratio','legacy_feasible_flag'});

engine_tbl = renamevars(engine_tbl, ...
    {'DG_rob','DA_rob','DT_rob','joint_margin','pass_ratio','feasible_flag'}, ...
    {'engine_DG_rob','engine_DA_rob','engine_DT_rob','engine_joint_margin','engine_pass_ratio','engine_feasible_flag'});

compare_tbl = innerjoin( ...
    legacy_tbl, engine_tbl, ...
    'Keys', {'design_id','h_km','i_deg','P','T','F','Ns'});

compare_tbl.DG_abs_diff = abs(compare_tbl.legacy_DG_rob - compare_tbl.engine_DG_rob);
compare_tbl.DA_abs_diff = abs(compare_tbl.legacy_DA_rob - compare_tbl.engine_DA_rob);
compare_tbl.DT_abs_diff = abs(compare_tbl.legacy_DT_rob - compare_tbl.engine_DT_rob);
...
disp('[manual] Stage09 ClosedD small-set comparison completed.');
disp(compare_tbl);
end
