function result = evaluate_single_layer_walker_stage05(row, trajs_in, gamma_req, cfg, hard_order, eval_context)
    %EVALUATE_SINGLE_LAYER_WALKER_STAGE05
    % Evaluate one Walker design point on nominal family using Stage03+Stage04 chain.
    %
    % Input:
    %   row       : one table row from Stage05 grid
    %   trajs_in  : trajbank.nominal from Stage02
    %   gamma_req : fixed threshold inherited from latest Stage04
    %   cfg       : default config
    %   hard_order: optional index order for nominal cases (hardest first)
    %
    % Output:
    ...
    %     rank_score
        ...
        % ------------------------------------------------------------
        % Build time grid from nominal trajectories
        % ------------------------------------------------------------
        ...
        for kk = 1:nCase
            ...
            pass_flag(k) = (D_G(k) >= cfg.stage05.require_D_G_min);
            ...
            % --------------------------------------------------------
            % Early stop:
            % if pass_ratio requirement is 1.0 and one case already fails,
            % this design is definitely infeasible.
            % --------------------------------------------------------
            if cfg.stage05.use_early_stop
                if cfg.stage05.require_pass_ratio >= 1.0 && (~pass_flag(k))
                    failed_early = true;
                    break;
                end
            end
        ...
        % fill unevaluated case_id strings for neatness
        for k = 1:nCase
        ...
        if isempty(D_G_valid)
            ...
            % important:
            % pass_ratio is computed over ALL nominal cases.
            % if early stop occurred, remaining cases are treated as fail
            % under the strict Stage05 criterion.
            if failed_early
        ...
        feasible_flag = (pass_ratio >= cfg.stage05.require_pass_ratio) && ...
                        (D_G_min >= cfg.stage05.require_D_G_min);
    
        switch string(cfg.stage05.rank_rule)
