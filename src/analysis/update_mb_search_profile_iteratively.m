...
if action.name == "hold" && ~quality.mid_transition_ok && ...
    ...
    desired_width = max(current_width, round(transition_width / max(0.35, target_center)));
    ...
    action.reason = "Shift the plotting window so the median transition band sits closer to the middle.";
end
