function traj = legacy_propagate_target_case_stage02_impl(case_i, cfg, hgv_cfg)
%LEGACY_PROPAGATE_TARGET_CASE_STAGE02_IMPL Copied Stage02 target propagator.
...
opts = odeset('RelTol', 1e-6, 'AbsTol', 1e-6, ...
    'Events', @(t, X) local_events_hgv_stage02_fast(X, event_params));

[T, X] = ode45(@(t, X) local_hgv_vtc_dynamics_const(X, ctrl.alpha_rad, ctrl.gamma_rad, p), ...
    [t0 tf], X0, opts);
...
function [value, isterminal, direction] = local_events_hgv_stage02_fast(X, event_params)
v = X(1);
phi = X(4);
lambda = X(5);
r = X(6);
