...
assert(numel(rows) == 9, 'Expected 9 expanded small-formal design rows.');

p_nom = make_profile_MB_nominal_small_formal();
p_head = make_profile_MB_heading_small_formal();

assert(numel(p_nom.design_pool.rows) == 9, 'Expected 9 nominal small-formal rows.');
assert(numel(p_head.design_pool.rows) == 9, 'Expected 9 heading small-formal rows.');
