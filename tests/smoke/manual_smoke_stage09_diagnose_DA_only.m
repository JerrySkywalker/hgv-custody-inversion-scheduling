function out = manual_smoke_stage09_diagnose_DA_only(cfg)
%MANUAL_SMOKE_STAGE09_DIAGNOSE_DA_ONLY
% Stage09 diagnosis smoke:
%   DG + DA + pass_ratio active
%   DT relaxed to 0
% Purpose:
%   identify which Stage05-compatible points are killed by D_A.
    ...
    cfg.stage09.run_tag = 'inverse_stage09_diagnose_DA_only';
    ...
    fprintf('\n[DA_ONLY] Running Stage09 diagnose smoke...\n');
    out = manual_smoke_stage09_stage05_aligned_fullscan(cfg);
    out.diag = stage09_dump_metric_diagnosis(out, 'DA_only', 15);

    fprintf('\n[DA_ONLY] Recommended CSVs:\n');
    fprintf('diag table   : %s\n', string(out.diag.files.diag_csv));
    fprintf('killed by DA : %s\n', string(out.diag.files.killed_da_csv));
