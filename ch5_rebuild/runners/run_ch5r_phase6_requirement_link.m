...
T = table( ...
    ...
    [req4.total_valid_steps; req5.total_valid_steps], ...
    ...
    [req4.observable_steps; req5.observable_steps], ...
...
lines{end+1} = ['This stage builds the minimal real-line bridge from bubble occurrence ' ...
                'to requirement-risk proxy violation.'];
lines{end+1} = '';
lines{end+1} = '## 2. Interpretation note';
lines{end+1} = '';
lines{end+1} = ['Current requirement-link analysis is proxy-based. It uses an inverse-information ' ...
                'risk proxy defined from the same rolling-window \lambda_{min}(J_W) quantity.'];
lines{end+1} = ['Therefore, the overlap between bubble and requirement-risk proxy violation is a ' ...
                'sanity check under a monotone proxy map, not an independent covariance-projection proof.'];
lines{end+1} = '';
lines{end+1} = '## 3. Key findings';
lines{end+1} = '';
lines{end+1} = ['- R4 valid-window req violation time = ', num2str(req4.total_violation_time_s, '%.6f'), ' s'];
lines{end+1} = ['- R5 valid-window req violation time = ', num2str(req5.total_violation_time_s, '%.6f'), ' s'];
lines{end+1} = ['- R4 mean margin proxy              = ', num2str(req4.mean_margin_proxy, '%.12g')];
lines{end+1} = ['- R5 mean margin proxy              = ', num2str(req5.mean_margin_proxy, '%.12g')];
lines{end+1} = '';
...
lines{end+1} = '';
lines{end+1} = '| policy | valid_steps | req_violation_steps | req_violation_time_s | req_violation_fraction | min_margin_proxy | mean_margin_proxy | observable_steps | bubble_req_overlap_ratio |';
lines{end+1} = '|---|---:|---:|---:|---:|---:|---:|---:|---:|';
for i = 1:height(T)
    lines{end+1} = ['| ', char(T.policy(i)), ...
        ...
        ' | ', num2str(T.bubble_req_overlap_ratio(i), '%.6f'), ' |'];
end
