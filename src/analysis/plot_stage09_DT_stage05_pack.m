function pack = plot_stage09_DT_stage05_pack(out_or_view, mode_tag)
%PLOT_STAGE09_DT_STAGE05_PACK
% Thin wrapper for DT Stage05-style ninepack plotting.
    ...
    pack = plot_stage09_metric_stage05_ninepack(metric_view.DT.table, metric_frontiers.DT, 'DT', cfg, mode_tag);
end
...
function [metric_view, metric_frontiers, cfg] = local_unpack_inputs(in)
    ...
    if ~isstruct(in)
        error('plot_stage09_DT_stage05_pack:InvalidInput', 'Input must be a struct.');
    end
    ...
    elseif isfield(in, 'views') && isfield(in, 'frontiers')
        ...
        error('plot_stage09_DT_stage05_pack:InvalidInput', ...
            ['Input must contain either:' newline ...
             '  (1) metric_view + metric_frontiers' newline ...
             '  (2) views + frontiers']);
    end

    if ~isfield(metric_view, 'DT') || ~isstruct(metric_view.DT) || ~isfield(metric_view.DT, 'table') || ~istable(metric_view.DT.table)
        error('plot_stage09_DT_stage05_pack:InvalidMetricView', ...
            'Expected metric_view.DT.table to be a table.');
    end
...
function cfg = local_pick_cfg(in)
    ...
    error('plot_stage09_DT_stage05_pack:MissingCfg', ...
