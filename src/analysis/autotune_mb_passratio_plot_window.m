...
state = "limit_reached";
stop_reason = "Reached the maximum number of auto-tune iterations.";
...
for iter = 1:max_iterations
    ...
    if local_profiles_equivalent(current_profile, next_profile)
        state = "stalled";
        stop_reason = "The iterative auto-tune update did not change the search profile.";
...
for idx = 1:numel(history)
    ...
    row_cells{idx, 1} = table( ...
        ...
        'VariableNames', { ...
            'iteration', 'action', 'action_reason', 'state', 'stop_reason', 'cache_hit', 'was_evaluated', ...
            'score', 'left_zero_score', 'right_one_score', 'transition_center_score', ...
            'monotonicity_soft_score', 'plateau_score', 'left_zero_reached', 'right_plateau_reached', ...
            'mid_transition_ok', 'full_transition_resolved', 'transition_ns_low', 'transition_ns_high', ...
            'final_passratio_median', 'P_grid', 'T_grid', 'plot_xlim_ns'});
end
