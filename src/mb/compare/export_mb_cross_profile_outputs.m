...
for idx_ctx = 1:size(contexts, 1)
    ...
    if ~isempty(pass_table)
        ...
        pass_global_csv = fullfile(paths.tables, sprintf('MB_profileCompare_%s_passratio_globalFullReplay_%s.csv', char(semantic_mode), context_tag));
        ...
        global_view_spec.figure_name = sprintf('MB_profileCompare_%s_passratio_globalFullReplay_%s.png', char(semantic_mode), context_tag);
        ...
        pass_global_png = fullfile(paths.figures, sprintf('MB_profileCompare_%s_passratio_globalFullReplay_%s.png', char(semantic_mode), context_tag));
        milestone_common_save_figure(fig_pass_global, pass_global_png);
        write_mb_plot_domain_sidecar(pass_global_png, "global_full_replay", "full_search_domain", global_xlim, ...
            ...
            'canonical_primary_mode', plot_mode_profile.canonical_primary_mode, ...
            'current_mode', "globalFullReplay", ...
            'is_primary_selection', plot_mode_profile.cross_profile_primary_mode == "globalFullReplay", ...
        ...
        primary_pass = pass_mode_files.(char(export_plot_plan.primary_cross_profile_view));
        ...
        pass_summary.global_full_replay_rendered_min_ns = repmat(local_pick_x(global_xlim, 1), height(pass_summary), 1);
        pass_summary.global_full_replay_rendered_max_ns = repmat(local_pick_x(global_xlim, 2), height(pass_summary), 1);
        ...
        artifacts.tables.(matlab.lang.makeValidName(sprintf('%s_passratioGlobalFullReplay_%s', char(semantic_mode), context_tag))) = string(pass_global_csv);
        ...
        artifacts.figures.(matlab.lang.makeValidName(sprintf('%s_passratioGlobalFullReplay_%s', char(semantic_mode), context_tag))) = string(pass_global_png);
