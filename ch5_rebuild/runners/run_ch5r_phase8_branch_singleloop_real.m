function out = run_ch5r_phase8_branch_singleloop_real()
%RUN_CH5R_PHASE8_BRANCH_SINGLELOOP_REAL
% Real single-loop branch for R8.6-real compare.
%
% Current minimal implementation:
%   - same truth/model stack as closedloop branch
%   - fixed pair = first selected pair from closedloop branch
%   - no outerB continuous reselection

closed = run_ch5r_phase8_5_outerB_continuous();
fixed_pair = closed.trace_data.selected_pair(1,:);

cfg = closed.cfg;
nx = 6;
ny = 3;

% rebuild truth from cfg
...
trace_data.selected_pair = repmat(fixed_pair, n_eval, 1);
...
out = struct();
out.name = 'R5-like_singleloop_real';
out.cfg = cfg;
