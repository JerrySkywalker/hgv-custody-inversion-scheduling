...
runtime_table = local_build_runtime_mode_summary(root_dir);
runtime_csv = fullfile(tables_dir, 'runtime_mode_summary.csv');
...
persistent session_initialized session_root session_paths session_elapsed_s init_count
...
if ~force_refresh && ~isempty(session_initialized) && session_initialized && strcmpi(session_root, root_dir)
    out = struct( ...
        ...
        'elapsed_s', 0, ...
        'initial_elapsed_s', session_elapsed_s, ...
        'init_count', local_get_default(init_count, 1));
    return;
end
...
tokens = ["outputs", "cache", "figures", "tables", "logs", "archive", "snapshots", ...
    "worktree", ".git", ".github", "temp", "tmp", "old", "backup", "docs", ...
    "delivery", "sandbox", "legacy_outputs", "__pycache__"];
tf = false;
...
function value = local_get_default(value_in, fallback)
if isempty(value_in)
    value = fallback;
else
    value = value_in;
...
function runtime_table = local_build_runtime_mode_summary(root_dir)
runtime = get_plot_runtime_config([]);
excluded_dirs_summary = strjoin([ ...
    "outputs", "cache", "figures", "tables", "logs", ".git", ".github", ...
    "temp", "tmp", "archive", "delivery", "sandbox", "legacy_outputs", ...
    "snapshots", "worktree", "__pycache__", "docs"], "; ");
runtime_table = table( ...
    true, ...
    runtime.mode ~= "visible", ...
    "appdata:groot_defaults", ...
    "whitelist_with_session_reuse", ...
    string(excluded_dirs_summary), ...
    string(root_dir), ...
    string(runtime.mode), ...
    logical(runtime.close_after_save), ...
    double(runtime.export_dpi), ...
    string(runtime.renderer), ...
    'VariableNames', { ...
        'startup_reuse_enabled', ...
        'figure_headless_default', ...
        'figure_runtime_source', ...
        'path_manager_mode', ...
        'excluded_dirs_summary', ...
        'root_dir', ...
        'figure_mode', ...
        'close_after_save', ...
        'export_dpi', ...
