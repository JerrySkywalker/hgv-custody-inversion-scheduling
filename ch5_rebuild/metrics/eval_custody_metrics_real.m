...
% Treat bubble occurrence as custody loss on the real R1.5/R3/R4 line.
%
% Important:
% Custody ratio is computed ONLY over valid_for_bubble samples.
...
if isfield(bubble, 'valid_for_bubble') && ~isempty(bubble.valid_for_bubble)
    ...
    valid_for_bubble = true(size(is_custody_lost));
end

assert(numel(is_custody_lost) == numel(valid_for_bubble), ...
    'bubble.is_bubble and bubble.valid_for_bubble size mismatch.');

masked_is_custody_lost = is_custody_lost & valid_for_bubble;

total_steps = numel(is_custody_lost);
total_valid_steps = nnz(valid_for_bubble);

loc_total_steps = nnz(masked_is_custody_lost);
loc_total_time_s = loc_total_steps * dt;

if total_valid_steps > 0
    custody_ratio = 1 - loc_total_steps / total_valid_steps;
else
    custody_ratio = NaN;
end

segments = local_find_segments(masked_is_custody_lost, dt);
if isempty(segments)
...
custody_metrics.total_valid_steps = total_valid_steps;
...
custody_metrics.valid_for_bubble = valid_for_bubble;
custody_metrics.is_custody_lost = masked_is_custody_lost;
