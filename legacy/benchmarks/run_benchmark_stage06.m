function report = run_benchmark_stage06(cfg)
    %RUN_BENCHMARK_STAGE06 Benchmark Stage06 heading-extended Walker search in serial vs parallel mode.
    ...
    [cfg, default_opts, default_mode] = rs_apply_parallel_policy('stage06', cfg, struct());
    [~, serial_opts] = rs_apply_parallel_policy('stage06', cfg, struct(), 'serial');
    [~, parallel_opts] = rs_apply_parallel_policy('stage06', cfg, struct(), 'parallel');

    cfg = local_apply_stage06_benchmark_overrides(cfg);
    ...
    bench_cfg = struct();
    bench_cfg.stage_name = 'stage06';
    bench_cfg.benchmark_name = 'stage06_heading_walker_search';
    bench_cfg.output_root = cfg.paths.benchmarks;
    bench_cfg.warmup_runs = cfg.benchmark.warmup_runs;
    bench_cfg.repeat = cfg.benchmark.stage06_repeat;
    ...
    bench_cfg.parallel_setup_fn = @local_prepare_stage06_parallel;
    bench_cfg.compare_opts = struct( ...
        ...
        'ignored_fields', {[cfg.benchmark.default_ignored_fields, {'cfg','log_file','table_file','feasible_table_file', ...
            'stage02_file','stage04_file','summary','grid','feasible_grid','eval_bank','trajs_heading'}]});
    bench_cfg.save_opts = struct( ...
        'save_mat', cfg.benchmark.save_mat, ...
        'save_json', cfg.benchmark.save_json);
    bench_cfg.input_summary_fn = @local_stage06_input_summary;
    bench_cfg.notes = 'Stage06 heading Walker search benchmark with fixed reduced grid and serial/parallel comparison.';

    report = run_solver_benchmark(@stage06_heading_walker_search, cfg, bench_cfg);

    fprintf('[benchmark] Stage06 primary view  : %s\n', report.timing.primary_view);
    fprintf('[benchmark] Stage06 cold serial   : %s\n', mat2str(report.timing.cold.serial_runs_s, 6));
    fprintf('[benchmark] Stage06 cold parallel : %s\n', mat2str(report.timing.cold.parallel_runs_s, 6));
    fprintf('[benchmark] Stage06 warm serial   : %s\n', mat2str(report.timing.warm.serial_runs_s, 6));
    fprintf('[benchmark] Stage06 warm parallel : %s\n', mat2str(report.timing.warm.parallel_runs_s, 6));
    fprintf('[benchmark] Stage06 parallel setup: %.6f s (excluded from run timing)\n', report.timing.parallel_setup_s);
    fprintf('[benchmark] Stage06 kernel prewarm serial   : %.6f s\n', report.timing.serial_kernel_prewarm_s);
    fprintf('[benchmark] Stage06 kernel prewarm parallel : %.6f s\n', report.timing.parallel_kernel_prewarm_s);
    fprintf('[benchmark] Stage06 serial best   : %.6f s\n', report.timing.serial_best_s);
    fprintf('[benchmark] Stage06 parallel best : %.6f s\n', report.timing.parallel_best_s);
    fprintf('[benchmark] Stage06 speedup(best) : %.4f x\n', report.timing.speedup_best);
    fprintf('[benchmark] Stage06 speedup(mean) : %.4f x\n', report.timing.speedup_mean);
    fprintf('[benchmark] Result check         : %s\n', report.compare.summary);
    fprintf('[benchmark] MAT report           : %s\n', report.paths.mat_file);
end

function cfg = local_apply_stage06_benchmark_overrides(cfg)
    cfg.stage06.use_live_progress = false;
    cfg.stage06.save_eval_bank = false;
    cfg.stage06.i_grid_deg = cfg.benchmark.stage06_i_grid_deg(:).';
    cfg.stage06.P_grid = cfg.benchmark.stage06_P_grid(:).';
    cfg.stage06.T_grid = cfg.benchmark.stage06_T_grid(:).';
    cfg.stage06.active_heading_set_name = 'custom';
    cfg.stage06.active_heading_offsets_custom_deg = cfg.benchmark.stage06_heading_offsets_deg(:).';
...
function summary = local_stage06_input_summary(cfg)
    summary = struct();
    ...
    summary.use_early_stop = cfg.stage06.use_early_stop;
    if isfield(cfg, 'run_stages') && isfield(cfg.run_stages, 'parallel_modes')
        summary.default_mode = cfg.run_stages.parallel_modes.stage06;
    end
end

function local_prepare_stage06_parallel(~, opts)
