function fig = plot_ch5b_speed_time(traj_samples, opts)
%PLOT_CH5B_SPEED_TIME Plot speed-time curves for one or more trajectories.
...
opts = apply_defaults(opts, struct( ...
    'visible', 'on', ...
    'line_width', 1.8, ...
    'title_text', 'ch5_bubble speed-time', ...
    'ylabel_text', 'Speed (m/s)'));
...
for i = 1:numel(traj_samples)
    ts = traj_samples(i);
    plot(ts.traj.t_s(:), ts.traj.v_mps(:), 'LineWidth', opts.line_width);
