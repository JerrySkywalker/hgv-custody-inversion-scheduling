...
global_full_replay_pass = ~isempty(passratio_table) && any(passratio_table.mode_name == "globalFullReplay") && ...
    all(logical(passratio_table.global_full_replay_pass(passratio_table.mode_name == "globalFullReplay")));
global_full_replay_primary_selected = local_primary_selection_pass(figures_dir, "globalFullReplay");
comparison_global_full_replay_export_pass = ~isempty(passratio_table) && any(passratio_table.semantic_name == "comparison" & passratio_table.mode_name == "globalFullReplay" & logical(passratio_table.global_full_replay_pass));
cross_profile_global_full_replay_export_pass = ~isempty(passratio_table) && ...
    any(contains(passratio_table.figure_name, "MB_profileCompare_", 'IgnoreCase', true) & passratio_table.mode_name == "globalFullReplay" & logical(passratio_table.global_full_replay_pass));
...
final_status = "fail";
if all([full_cache_rebuild_pass, passratio_semantics_pass, heatmap_semantics_pass, history_no_zero_padding_pass, effective_dense_rebuild_pass, global_full_replay_pass, global_full_replay_primary_selected, comparison_global_full_replay_export_pass, cross_profile_global_full_replay_export_pass, full_heights_run_pass, strict_replica_pass, stage05_06_core_untouched])
    final_status = "pass";
end
...
closure_table = table( ...
    ...
    logical(global_full_replay_pass), ...
    logical(global_full_replay_primary_selected), ...
    logical(comparison_global_full_replay_export_pass), ...
    logical(cross_profile_global_full_replay_export_pass), ...
    logical(full_heights_run_pass), ...
    logical(strict_replica_pass), ...
    logical(stage05_06_core_untouched), ...
    string(final_status), ...
    'VariableNames', {'full_cache_rebuild_pass', 'passratio_semantics_pass', 'heatmap_semantics_pass', 'history_zero_padding_removed_pass', 'effective_dense_rebuild_pass', 'global_full_replay_pass', 'global_full_replay_primary_selected', 'comparison_global_full_replay_export_pass', 'cross_profile_global_full_replay_export_pass', 'full_heights_run_pass', 'strict_replica_pass', 'stage05_06_core_untouched', 'final_status'});
end
