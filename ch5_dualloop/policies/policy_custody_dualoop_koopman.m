function result = policy_custody_dualoop_koopman(caseData, cfg)
%POLICY_CUSTODY_DUALOOP_KOOPMAN
% NX-2 first round
% CK policy with minimal state-machine switching:
%   - dwell
%   - hysteresis
%   - guard
%
% If state-machine is disabled by ablation flags, fallback to fixed warn mode.
...
for k = 1:N
    ...
    if disable_state_machine
        ...
        [do_switch, ~] = should_switch_under_guard_minimal(sm_prev, sm_now, ttl_steps_now, k, cfg);
    ...
    if do_switch
        ids = select_satellite_set_custody_dualoop(caseData, k, prev_ids, char(mode), cfg);
