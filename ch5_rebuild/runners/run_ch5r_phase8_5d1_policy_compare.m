...
% R8.5d.1:
%   Compare three policy traces on unified lightweight bubble-effect chain:
%     1) PTA
%     2) observability_family (CN proxy)
%     3) current_method (R5-real selection_trace)
...
disp(' ')
disp('=== [ch5r:R8.5d.1] policy compare summary ===')
disp(compare_table)
...
lines = {};
lines{end+1} = '# Phase R8.5d.1 policy compare';
lines{end+1} = '';
...
for i = 1:height(compare_table)
    lines{end+1} = sprintf('- %s: bubble_steps=%g, bubble_time_s=%g, switch_count=%g, resource_score=%g, mean_MG_proxy=%g', ...
