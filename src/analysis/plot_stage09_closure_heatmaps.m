function out = plot_stage09_closure_heatmaps(base, mode_tag)
%PLOT_STAGE09_CLOSURE_HEATMAPS
% Phase4-B:
% Plot four-layer closure heatmaps at a selected h-slice.
%
% Layers:
...
%   4. DT_best
    ...
    if nargin < 2 || isempty(mode_tag)
        mode_tag = 'phase4_closure';
    end

    if ~isstruct(base) || ~isfield(base, 'cubes')
        error('plot_stage09_closure_heatmaps:InvalidInput', ...
    ...
    layer_specs = { ...
        struct('layer_name','joint_feasible_ratio','title','Joint feasible ratio over (i,P)','cbar','Joint feasible ratio'), ...
        struct('layer_name','DG_best','title','DG best over (i,P)','cbar','DG best'), ...
        struct('layer_name','DA_best','title','DA best over (i,P)','cbar','DA best'), ...
        struct('layer_name','DT_best','title','DT best over (i,P)','cbar','DT best') ...
    };
    ...
    for k = 1:numel(layer_specs)
        ...
        ylabel(ax, 'P');
        title(ax, sprintf('%s (h = %g km)', spec.title, h_vals(h_idx)));
        grid(ax, 'on');
        ax.GridAlpha = 0.18;
        ax.LineWidth = 0.8;
        ax.Layer = 'top';
    ...
    title(tl, sprintf('Stage09 closure heatmaps at h = %g km', h_vals(h_idx)), 'FontWeight', 'bold');

    fig_name = sprintf('stage09_closure_heatmaps_%s_%s_%s.png', run_tag, mode_tag, timestamp);
    ...
    figure_index = table( ...
        ...
        h_vals(h_idx), ...
        string(fig_path), ...
        'VariableNames', {'run_tag','mode_tag','timestamp','h_km','fig_closure_heatmaps'});

    figure_index_csv = fullfile(out_dir_tbl, ...
        sprintf('stage09_closure_heatmaps_figure_index_%s_%s_%s.csv', run_tag, mode_tag, timestamp));
    ...
    fprintf('\n');
    fprintf('================ Stage09 Closure Heatmaps Summary ================\n');
    fprintf('run_tag      : %s\n', run_tag);
    fprintf('mode_tag     : %s\n', mode_tag);
...
function [out_dir_fig, out_dir_tbl] = local_resolve_output_dirs(base)
    ...
    if isstruct(base) && isfield(base, 'cfg') && isstruct(base.cfg) ...
        ...
        if isfield(base.cfg.paths, 'outputs') && isstruct(base.cfg.paths.outputs)
            if isfield(base.cfg.paths.outputs, 'stage09_figs')
                out_dir_fig = fullfile(base.cfg.paths.outputs.stage09_figs, 'closure_heatmaps');
            end
            if isfield(base.cfg.paths.outputs, 'stage09_tables')
                out_dir_tbl = fullfile(base.cfg.paths.outputs.stage09_tables, 'closure_heatmaps');
            end
        end
    end
    ...
    if isempty(out_dir_fig) || isempty(out_dir_tbl)
        ...
        if isempty(out_dir_fig)
            out_dir_fig = fullfile(project_root, 'outputs', 'stage', 'stage09', 'figs', 'closure_heatmaps');
        end
        if isempty(out_dir_tbl)
            out_dir_tbl = fullfile(project_root, 'outputs', 'stage', 'stage09', 'tables', 'closure_heatmaps');
        end
    end
end
...
function value = local_pick_first_existing_field(s, names, default_value)
    ...
    error('plot_stage09_closure_heatmaps:MissingField', ...
...
function idx = local_find_name(names, target)
    ...
    if isempty(idx)
        error('plot_stage09_closure_heatmaps:NameNotFound', ...
