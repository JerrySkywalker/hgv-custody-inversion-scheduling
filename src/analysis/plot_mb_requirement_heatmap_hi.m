...
if nargin >= 2 && ~isempty(minimum_design_table)
    scatter(ax, minimum_design_table.i_deg, minimum_design_table.h_km, 120, 'p', ...
...
xlabel(ax, 'i (deg)');
ylabel(ax, 'h (km)');
title(ax, sprintf('Minimum Feasible Constellation Requirement over (h, i)\nDiscrete field on the current MB design-pool projection'));
grid(ax, 'on');
hold(ax, 'off');
end
